...
(function (window, undefined) {
    ...
    var Keycloak = function (config) {
        ...
        kc.handleCustomURICallback = function (url) {

            if (url === adapter.redirectUri()) {
                kc.clearToken();
            } else {
                ...
                initPromise.promise.success(function () {
                    ...
                    kc.onReady && kc.onReady(kc.authenticated);
                    if (!kc.authenticated) {
                        kc.onAuthError && kc.onAuthError(kc.authenticated)
                    }
                }).error(function (errorData) {
                    console.log("Error while procesing sucessfull login callback " + errorData)
                });
                processCallback(callback, initPromise);
            }
            ...
            if (type == 'cordova') {
                ...
                return {
                    ...
                    login: function (options) {
                        ...
                        var hideBrowser = options.prompt && options.prompt === "none" ? true : false;
                        var loginUrl = kc.createLoginUrl(options);
                        var completed = false;
                        //Use SafariController to share KC cookies across multiple mobile app
                    ...
                    logout: function (options) {
                        ...
                        SafariViewController.isAvailable(function (available) {
                            if (available) {
                                ...
                                var ref = window.open(logoutUrl, '_system', 'location=no,hidden=yes');
                            }
                        })
                        return promise.promise;
                    },
                    ...
                    register: function () {
                        ...
                        SafariViewController.isAvailable(function (available) {
                            if (available) {
                                ...
                                // potentially powered by InAppBrowser because that (currently) clobbers window.open
                                var ref = window.open(registerUrl, '_system', 'location=no');
                    ...
                    accountManagement: function () {
                        ...
                        SafariViewController.isAvailable(function (available) {
                            if (available) {
                                ...
                                // potentially powered by InAppBrowser
                                var ref = window.open(accountUrl, '_system', 'location=no');
                            }
                        })
